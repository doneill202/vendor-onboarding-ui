...
async function bootstrap(){
  try{
    ...
    if(!vendorToken){
      ...
      app.appendChild(box);
      return; // stop initialization, do not create a draft
    }

    state.vendorToken = vendorToken; state.inviterEmail = inviterEmail;

    // Reference data + initialize draft
    ...
    // Capture invite fields if backend returns them (preferred), else fallbacks
...
const pages = {
  ...
  1: function profile(){
    ...
    // placeholder updated per request
  ...
  // 3) Contacts (prefill from invite if no contacts yet)
  3: function contacts(){
    ...
    // Prefill once if there are no contacts yet
    try{
