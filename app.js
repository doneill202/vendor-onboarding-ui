...
function routeTo(stepId){ 
 ...
 // NEW: always scroll to top on page load
...
const pages = { 
 ...
 await savePage(state.draftId, 1, p); state.step = Math.max(state.step, 1); routeTo(2); 
 }})); 
 }, 
 2: function sites(){ 
 ...
 p.contacts.forEach((c,i)=>{ const tr=el('tr', {}, 
 ...
 el('td', {}, el('button',{class:'btn',onclick:()=>{p.contacts.splice(i,1);render();}},'Remove')) ); tbody.appendChild(tr); }); }
 ...
 // NEW: Add button on its own row below the flags
 ...
 app.appendChild(header); app.appendChild(el('div',{class:'card'}, info, file, status)); 
 ...
 const f=file.files[0]; if(f.type!=='application/pdf'){ alert('Please upload a PDF'); return; } if(f.size>SETTINGS.maxUploadBytes){ alert('File exceeds 10MB limit'); return; } 
 const sas=(await getSasForTax(f.name,f.type,f.size,state.vendorToken)).data || (await getSasForTax(f.name,f.type,f.size,state.vendorToken)); await uploadWithSas(sas.uploadUrl,f); p.taxDoc={ fileName:f.name, stagingPath:sas.stagingPath }; state.payload=state.payload||{}; state.payload.page4=p; saveLocal(); await savePage(state.draftId,4,p); routeTo(5); 
 }})); 
 }, 
 ...
 function buildMulti(){
 // Rebuild all three sections so Check All buttons reflect immediately
 ...
 function chkAll(listSetter, list){ listSetter(list.map(x=>x.id)); routeTo(7); }
 app.appendChild(el('div',{class:'card'}, el('h2',{},'Capabilities'), el('p',{class:'help'},'Please describe your advertising capabilities.'))); 
 app.appendChild(el('div',{class:'card'}, el('h3',{},'Ad Types (please select at least one)'), ad, el('div',{class:'row'}, el('button',{class:'btn', onclick:()=> chkAll(ids=>{p.adTypeIds=ids;}, adList)}, 'Check All') ))); 
 app.appendChild(el('div',{class:'card'}, el('h3',{},'Pricing Types (please select at least one)'), pricing, el('div',{class:'row'}, el('button',{class:'btn', onclick:()=> chkAll(ids=>{p.pricingTypeIds=ids;}, pricingList)}, 'Check All') ))); 
 app.appendChild(el('div',{class:'card'}, el('h3',{},'Targeting (please select at least one)'), targeting, el('div',{class:'row'}, el('button',{class:'btn', onclick:()=> chkAll(ids=>{p.targetingIds=ids;}, targList)}, 'Check All') ))); 
 app.appendChild(el('div',{class:'card'}, el('h3',{},'Campaign Functionality (please select at least one)'), camp, el('div',{class:'row'}, el('button',{class:'btn', onclick:()=> chkAll(ids=>{p.campaignFunctionalityIds=ids;}, campList)}, 'Check All') ))); 
 app.appendChild(el('div',{class:'card'}, el('h3',{},'Regions (please select at least one)'), regions, el('div',{class:'row'}, el('button',{class:'btn', onclick:()=> chkAll(ids=>{p.regionIds=ids;}, regionList)}, 'Check All') ))); 
