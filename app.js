...
function routeTo(stepId){
  ...
  // Keep scroll-to-top on page navigation only
...
const pages = {
  ...
  1: function profile(){
    ...
    // Helper/placeholder updated per request
  ...
  // 2) Sites (unchanged from last push aside from spacing and empty text)
  2: function sites(){
  ...
  // 3) Contacts (unchanged from your approved layout tweaks)
  3: function contacts(){
    ...
    app.appendChild(el('div', { class:'card' },
      ...
      el('div', { class:'row kv', style:'margin-bottom:8px;' }, ph,
  ...
  // 5) Demographics with Check All (already implemented previously)
  ...
  // 6) Interests with Check/Uncheck All (already added)
  ...
  // 7) Capabilities — FIX: section-local Check All without page jump
  7: function capabilities(){
    ...
    // section containers so we can rebuild in-place (no routeTo, no scroll jump)
    const sectAd  = el('div');
    const sectPr  = el('div');
    const sectTa  = el('div');
    const sectCa  = el('div');
    ...
    function buildCaps(){
      // Clear & rebuild each section based on current arrays
  ...
  // 8) Review (unchanged from last push)
  8: function review(){
