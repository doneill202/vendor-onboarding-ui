...
function renderNav(){ nav.innerHTML=''; steps.forEach(s=>{
...
function routeTo(stepId){ state.step = stepId; renderNav(); window.location.hash = '#'+stepId; app.innerHTML='';
...
function btnRow(prev, next, {hidePrev=false, onPrev=null, onNext=null, nextText='Save & Next'}={}){
  ...
  if(!hidePrev){ const b=el('button',{class:'btn ghost',onclick:()=>{ if(onPrev) onPrev(); else routeTo(prev); }},'Back'); row.appendChild(b); }
...
const pages={
  1: function profile(){
    ...
    const desc = el('textarea'); desc.value = p.description||''; desc.addEventListener('input', ev=>p.description=ev.target.value);
    app.appendChild(el('div',{class:'card'}, el('h2',{},'Company Profile')));
    app.appendChild(inputRow('Company Name *', nameInput));
    app.appendChild(inputRow('Website *', webInput));
    app.appendChild(inputRow('Description', desc));
    ...
    app.appendChild(btnRow(1,2,{hidePrev:true, nextText:'Save & Continue', onNext: async()=>{
      ...
      if(!p.companyName || !p.website){ alert('Please provide Company Name and Website.'); return; }
      state.payload = state.payload||{}; state.payload.page1 = p; saveLocal();
      const res = await savePage(state.draftId, 1, p); state.step=Math.max(state.step,1); routeTo(2);
    }}));
  },
  2: function sites(){
    ...
    function render(){
      tbody.innerHTML='';
      p.sites.forEach((s,i)=>{
        const tr=el('tr',{}, el('td',{},s.siteName||''), el('td',{},s.url||''), el('td',{}, el('button',{class:'btn',onclick:()=>{p.sites.splice(i,1); render();}},'Remove')));
        tbody.appendChild(tr);
      });
    }
    const nameI=textField({}); const urlI=textField({placeholder:'https://...'});
  ...
  3: function contacts(){
    ...
    function render(){
      ...
      p.contacts.forEach((c,i)=>{
        const tr=el('tr',{}, el('td',{},`${c.firstName||''} ${c.lastName||''}`.trim()), el('td',{},c.email||''), el('td',{}, c.isPrimary?'Primary':''), el('td',{}, c.isAccounting?'Accounting':''), el('td',{}, el('button',{class:'btn',onclick:()=>{p.contacts.splice(i,1); render();}},'Remove')) );
        tbody.appendChild(tr);
      });
    }
    const f=textField({placeholder:'First name'}), l=textField({placeholder:'Last name'}), e=textField({type:'email',placeholder:'email@domain.com'}), ph=textField({placeholder:'Phone (optional)'});
    const prim=el('input',{type:'checkbox'}), acct=el('input',{type:'checkbox'});
    const add=el('button',{class:'btn',onclick:()=>{ if(!f.value||!l.value||!e.value){alert('Provide first, last, and email');return;} p.contacts.push({firstName:f.value,lastName:l.value,email:e.value,phone:ph.value||'',isPrimary:prim.checked,isAccounting:acct.checked}); f.value=l.value=e.value=ph.value=''; prim.checked=acct.checked=false; render(); }},'Add');
    ...
    app.appendChild(el('div',{class:'card'}, el('h2',{},'Contacts'), el('div',{class:'row'}, f,l,e,ph, el('label',{}, el('input',{type:'checkbox',onchange:ev=>{prim.checked=ev.target.checked}}),' Primary'), el('label',{}, el('input',{type:'checkbox',onchange:ev=>{acct.checked=ev.target.checked}}),' Accounting'), add ), table));
  ...
  4: function tax(){
    ...
    app.appendChild(btnRow(3,5,{onPrev:()=>routeTo(3), nextText:'Save & Continue', onNext: async()=>{
      if(!file.files?.length){ if(SETTINGS.taxOptional){ await savePage(state.draftId,4,p); routeTo(5); return; } else { alert('Please upload a PDF.'); return; } }
  ...
  5: function demographics(){
    ...
    const pfVal=el('span',{class:'badge'}, (p.percentFemale??50)+'%'); pf.addEventListener('input',ev=>{ p.percentFemale=parseInt(ev.target.value,10); pfVal.textContent=p.percentFemale+'%'; });
  ...
  8: function review(){
    ...
    app.appendChild(box); app.appendChild(el('div',{class:'row'}, el('button',{class:'btn',onclick:()=>routeTo(7)},'Back'), submit));
  }
};
